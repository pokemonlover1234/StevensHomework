...
and
  ...
  | Proj(e,id) ->
    let rec fieldGet : exp_val -> exp_val ea_result = (function
    | RecordVal ([]) -> error "Field not found!"
    | RecordVal ((field,(_,value))::t) -> if (field = id) then return value else fieldGet (RecordVal t)
    | _ -> error "Expected a record!"
    ) in eval_expr e >>= fieldGet >>= fun field -> (
      match field with
  ...
  | SetField(e1,id,e2) ->
    let rec fieldGet : exp_val -> exp_val ea_result = (function
    | RecordVal ([]) -> error "Field not found!"
    | RecordVal ((field,(mut,value))::t) -> if (field = id) then (if (mut) then return value else error "Field not mutable!") else fieldGet (RecordVal t)
    | _ -> error "Expected a record!"
    ) in eval_expr e1 >>= fieldGet >>= int_of_refVal >>= fun field ->
      eval_expr e2 >>= Store.set_ref g_store field >>= fun _ ->
      return UnitVal
  | Unit -> return UnitVal
  | Debug(_e) ->
