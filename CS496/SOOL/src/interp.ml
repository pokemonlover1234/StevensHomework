...
let lookup_class : string -> class_env -> class_decl ea_result  = fun c_name c_env ->
  let decl = List.assoc_opt c_name c_env in match decl with
  | None -> error "lookup_class: class c_name not found"
  | Some decl -> return decl
    
let new_env : string list -> env ea_result  = fun fs ->
  let rec new_env' fs acc =
    match fs with
    | [] -> acc 
    | h::t -> new_env' t (ExtendEnv(h, RefVal(Store.new_ref g_store (NumVal 0)), acc))
  in return @@ new_env' fs EmptyEnv
...
(*
Since we use this method on a class that we know exists,
since the class is gotten from an existing object, no need
for assoc_opt
*)
let lookup_method : string -> string -> class_env ->
  method_decl option = fun c_name m_name c_env ->
  let c = List.assoc c_name c_env in match c with
  | (_,_,m_env) -> List.assoc_opt m_name m_env
...
and
  ...
  | NewObject(c_name,es) ->
    ...
    lookup_class c_name !g_class_env >>= fun decl ->
    (match decl with
    | (_,fs,ms) -> new_env fs >>= fun env ->
      let obj = ObjectVal(c_name,env) in
      let mopt = List.assoc_opt "initialize" ms in (
        match mopt with
        | None -> return obj
        | Some m -> apply_method "initialize" obj evs m >>= fun _ ->
          return obj
        )
    )
  | Send(e,m_name,es) ->
