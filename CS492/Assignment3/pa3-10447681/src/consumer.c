...
static int do_procs(int fd) {
	int i, status, ret = 0;
	pid_t pid;
	long elemsz = ioctl(fd, SCULL_IOCGETELEMSZ);
	char * buf;
	int count;

	/* Guarantee all 0s so we don't get garbage data if not all of 
	   an element is filled. Also guarantees a \0 byte after element
	   data. */
	buf = calloc(elemsz + 1, sizeof(char));
