...
%% executes `/join` protocol from client perspectives
do_join(State, Ref, ChatName) ->
    case maps:is_key(ChatName, State#cl_st.con_ch) of
		...
		false ->
			...
			receive
				{ChatroomPID, Ref2, connect, History} ->
					{{ref, Ref2, History}, State#cl_st{con_ch=maps:put(ChatName, ChatroomPID, State#cl_st.con_ch)}}
			end
	end.
...
do_leave(State, Ref, ChatName) ->
	case maps:is_key(ChatName, State#cl_st.con_ch) of
		...
		true ->
			...
			receive
				{_ChatroomPID, Ref2, ack_leave} ->
					{{ref, Ref2, ok}, State#cl_st{con_ch=maps:remove(ChatName, State#cl_st.con_ch)}}
			end
	end.
...
do_new_nick(State, Ref, NewNick) ->
    if
		...
		true ->
			...
			receive
				{_ServerPID, Ref2, err_nick_used} ->
					{{ref, Ref2, err_nick_used}, State};
				{_ServerPID, Ref2, ok_nick} ->
					{{ref, Ref2, ok_nick}, State#cl_st{nick=NewNick}}
			end
	end.
...
do_msg_send(State, Ref, ChatName, Message) ->
	...
	receive
		{_Chatroom, Ref2, ack_msg} ->
			{{ref, Ref2, {msg_sent, State#cl_st.nick}}, State}
	end.
...
do_quit(State, Ref) ->
	...
	receive
		{_Server, Ref2, ack_quit} ->
			whereis(list_to_atom(State#cl_st.gui))!{self(), Ref2, ack_quit},
			{shutdown, State}
	end.