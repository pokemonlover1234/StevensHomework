...
private:
    ...
    void delete_tree(Node<K, V> *n) {
        if(n){
            //Recursively traverse the tree to delete the nodes
            delete_tree(n->left);
            delete_tree(n->right);
            delete n;
        }
    ...
    void insert_fixup(Node<K, V> *z) {
        Node<K,V> *y;
        while(z->parent ? (z->parent->color == RED) : false){
            if((z->parent) ? (z->parent->parent ? (z->parent == z->parent->parent->left) : false) : true){
                y = z->parent->parent->right;
                if(y ? (y->color == RED) : false){
    ...
    int height(Node<K, V> *node) const {
        if(node){
            //The height of the tree is the maximum height
    ...
    size_t leaf_count(Node<K, V> *node) const {
        if(node){
            //Traverse the tree, adding 1 if the node is a leaf
            return leaf_count(node->left) + leaf_count(node->right) + ((!node->right && !node->left) ? 1 : 0);
        }else{
            return 0;
        }
    }
    ...
    size_t internal_node_count(Node<K, V> *node) const {
        if(node){
            //Get the internal node counts for each child, and add 1 if this node is itself an internal node.
            return internal_node_count(node->left) + internal_node_count(node->right) + ((node->right || node->left) ? 1 : 0);
        }else{
            return 0;
        }
    }
    ...
    int diameter(Node<K, V> *node) const {
        if(node){
            //The diameter of a tree is the maximum between the diameters of the left and right subtrees, and the length of the longest path
            //between leaves including node, which is the sum of the heights of the childs + 1 for itself.
            //It's +3 on the heights to account for the -1 of null node heights.
            return std::max(height(node->left) + height(node->right) + 3, std::max(diameter(node->left),diameter(node->right)));
        } else {
            return 0;
        }
    }
    ...
    size_t width(Node<K, V> *node, size_t level) const {
        if(level == 0){
            //If level is 0 (we're at the requested level), add this node to the count if it isn't a null node.
            return node ? 1 : 0;
        }else if(node){
            //Recursively traverse the tree until we reach the requested level.
            //Subtract 1 from level each time, because from the perspective of the child node, the requested level x is level x - 1
    ...
    size_t null_count(Node<K, V> *node) const {
        if(node){
            ...
            //If node is nullptr, then we found a null node, so add one to the count.
            return 1;
        }
    }
    ...
    size_t sum_levels(Node<K, V> *node, size_t level) const {
        if(node){
            //Traverse the tree, adding the level of the node currently in question
    ...
    size_t sum_null_levels(Node<K, V> *node, size_t level) const {
        if(node){
            ...
            //If the node is null, return the level. Since each node at each level that is null will get here, 
            //the required result will be reached.
            return level;
        }
    }
};
