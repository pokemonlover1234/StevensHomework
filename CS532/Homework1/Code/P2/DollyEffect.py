...
def output_video():
    ...
    vw = cv2.VideoWriter("../Output/P2/dolly.wmv", 0, 15, (3073, 2049))
    for step in range(FPS * LEN):
        vw.write(cv2.imread("P2/RawFrames/{}.jpg".format(step)))


# Modified version of the sample function provided, with dolly
# zoom effect included.
def dolly_effect():
    # load object file to retrieve data
    file_p = open("P2/data.obj", 'rb')
    ...
    R = np.identity(3)
    move = np.array([0, 0, -2/(FPS * LEN)]).reshape((3, 1))

    for step in range(FPS * LEN):
        fname = "P2/RawFrames/{}.jpg".format(step)
        print("\nGenerating {}".format(fname))
        t = step*move
        t[2] -= 1.25
        if step != 0:
            K[0, 0] *= (FOCAL_Z_INIT - 2 / (FPS * LEN) * step) / \
                (FOCAL_Z_INIT - 2/(FPS * LEN) * (step - 1))
            K[1, 1] *= (FOCAL_Z_INIT - 2 / (FPS * LEN) * step) / \
                (FOCAL_Z_INIT - 2/(FPS * LEN) * (step - 1))
